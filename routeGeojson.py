...
def pythagoras(x1, y1, x2, y2):
    # Not really correct for world geometry,
    # but good enough for what we're doing with it.
    x_d = abs(x1 - x2)
    y_d = abs(y1 - y2)
...
def route_geojson(input_f, output_f, mode='foot', local_planet=None):
    ...
    print(
        ...
        (successful, non_linestring, not_two_points,
         very_long, unsuccessful)
    )
...
def main():
    ...
    parser.add_argument(
        '-l', '--local-planet',
        help='Use a local OSM XML file. '
        'Make sure to crop it for good performance!')
    options = parser.parse_args()
    route_geojson(
        options.input[0],
        options.output,
        options.mode,
        options.local_planet)
